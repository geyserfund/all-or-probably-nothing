...
if (fs.existsSync('keys.txt')) {
  ...
  fs.writeFileSync('keys.txt', texttowrite, function () {
    return;
  });
}
...
function encrypt(privkey, pubkey, text) {
  ...
  var cipher = browserifyCipher.createCipheriv('aes-256-cbc', Buffer.from(key, 'hex'), iv);
...
function decrypt(privkey, pubkey, ciphertext) {
  ...
  var decipher = browserifyCipher.createDecipheriv('aes-256-cbc', Buffer.from(key, 'hex'), Buffer.from(iv, 'base64'));
...
async function getNote(id) {
  ...
  var note = '';
  socket.on('message', async function (_event) {
...
async function sendDM(note, recipient) {
  ...
  socket.on('open', function open() {
    function makePrivateNote(note, recipientpubkey) {
      ...
      var newevent = [0, pubKeyMinus2, now, 4, [['p', recipientpubkey]], privatenote];
      var message = JSON.stringify(newevent);
      var msghash = sha256(message);
      nobleSecp256k1.schnorr.sign(msghash, privKey).then((value) => {
        var sig = value;
        nobleSecp256k1.schnorr.verify(sig, msghash, pubKeyMinus2).then((value) => {
          if (value) {
            var fullevent = {
              id: msghash,
              pubkey: pubKeyMinus2,
...
async function handleMessage(_event) {
  var event = JSON.parse(_event);
  ...
  socket.on('message', async function (_event) {
    ...
    if (event[2] && event[2].kind == 4239) {
      ...
      if (json['type'] == 'funding info') {
        ...
        raised_array[subscription_id]['raised'] =
          Number(raised_array[subscription_id]['raised']) + Number(value_in_dollars);
  ...
  if (json['type']) {
    ...
    if (json['type'] == 'over') {
      ...
      handlePublicMessages(
        ...
        subId2,
        hash,
        JSON.parse(contents)['creator_pubkey'],
        JSON.parse(contents)['goal'],
        JSON.parse(contents)['timestamp']
      );
    ...
    if (
      ...
      json['tx'] &&
      isHex(json['tx']) &&
      json['tx'].length < 50000 &&
      json['time'] &&
      typeof json['time'] == 'number' &&
      String(json['time']).length == 10
    ) {
      ...
      fs.writeFileSync('watchtower.txt', texttowrite, function () {
        return;
      });
    }
  }
...
function watchTower() {
  ...
  if (fs.existsSync('watchtower.txt')) {
    var watchtowertext = fs.readFileSync('watchtower.txt').toString();
    txs = JSON.parse(watchtowertext);
  } else {
    txs = [];
    texttowrite = JSON.stringify(txs);
    fs.writeFileSync('watchtower.txt', texttowrite, function () {
      return;
    });
  }
