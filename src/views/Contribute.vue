...
async function theCreatorDidOnlyOneFundraiser(creator) {
  var subId3 = '00000000' + ECPair.makeRandom().privateKey.toString('hex').substring(8);
...
function generateFundraiserScript(timestamp, contributorkey, oracle_hash, creatorkey) {
  return bitcoinjs.script.fromASM(
    makeFundraiserScript(timestamp, contributorkey, oracle_hash, creatorkey).trim().replace(/\s+/g, ' ')
  );
}
...
function prepareTx(txid, txindex, amount, myaddress, privkey, timestamp, contributorkey, oracle_hash, creatorkey) {
  ...
  psbt.addInput({
    ...
    witnessUtxo: {
      script: Buffer.from('0020' + bitcoinjs.crypto.sha256(Buffer.from(witnessscript, 'hex')).toString('hex'), 'hex'),
...
async function showQR(message) {
  ...
  document.getElementById(
    'modal'
  ).innerHTML += `<div align="center" style="overflow-wrap: break-word; margin-bottom: 20px; font-size: .8em;">${message}</div>`;
...
function txsubmit(txid, txindex, amount) {
  ...
  console.log('tx:', tx);
  console.log(
    `prepareTx( "${txid}", ${txindex}, ${amount}, "${myaddress}", "${privkey}", ${timestamp}, "${contributorkey}", "${oracle_hash}", "${creatorkey}" )`
  );
  ...
  document.getElementById(
    'modal'
  ).innerHTML += `<div align="center" style="overflow-wrap: break-word; margin-bottom: 20px; font-size: .8em;">${alert}</div>`;
  document.getElementById('black-bg').style.display = 'block';
  document.getElementById('modal').style.display = 'block';
}
...
<template>
  <div>
    ...
    <div id="meaningful_content" style="display: none">
      ...
      <p id="message"></p>
      <input id="address" type="text" placeholder="your refund address" name="address" />
      <p>
        <button
          id="submitter"
          type="button"
          name="submitter"
          onclick='showQR( sessionStorage[ "fundraiser_address" ]);'
        >
          Submit
        </button>
      </p>
      <p>
        ⚠️ You are about to see a bitcoin smart contract address, do not share it! It is for your eyes only. Other
        contributors will see different addresses. If more than one person sends money to your smart contract address
        their money could be lost forever.
      </p>
      <h2>Progress bar <span id="goal" style="font-size: 0.8em; font-weight: normal"></span></h2>
      <div id="progressOutline" style="height: 2em; border: 1px solid grey; border-radius: 25px; overflow: hidden">
        <div id="progressBar" style="height: 2em; background-color: #61eb34; width: 0%; transition: width 1s"></div>
      </div>
      <div>
        <div id="progressIndicator" style="display: inline-block; width: 0%; transition: width 1s"></div>
        <div id="total" style="display: inline-block; text-align: right">raised so far: $0</div>
      </div>
